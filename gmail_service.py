...
def get_message_snippit(message_id):
    svc = get_email_service()
    message = svc.users().messages().get(userID="me", id=message_id, format="full").execute()
    return message["snippit"], message["threadID"]

def get_thread(thread_id):
    svc = get_email_service()
    thread = svc.users().threads().get(userID="me", id=thread_id, format="full").execute()
    return [m["snippit"] for m in thread["messages"]]
...
def send_email(to: str, subject: str, body: str):
    ...
    raw = base64.urlsafe_b64decode(mime.as_bytes()).decode()
